...
import { GroupUpdatedListener } from './events/listeners/GroupUpdatedListener';
...
const start = async () => {
  try {
    ...
    new GroupUpdatedListener(natsWraper.client).listen();
    ...
    app.listen(5002);
    console.log('Connected to Category Service, listening on PORT: 5002');
  } catch (error) {
    console.log(error);
  }
};
